...
#
# 支持 task.executor=qoder_cli / qoder_cli_cn / kimi_cli。
#
...
#   bash scripts/build-sandbox-image.sh --no-qoder-cli --no-kimi-cli # 仅国内版(零 Node 依赖)
#
...
for arg in "$@"; do
    case "$arg" in
        ...
        --no-kimi-cli)
            ...
            ;;
        -h|--help)
            echo "用法:bash $0 [--with-qoder-cli|--no-qoder-cli] [--with-qoder-cli-cn|--no-qoder-cli-cn] [--with-kimi-cli|--no-kimi-cli]"
            echo ""
            echo "选项(均可组合,默认全部启用):"
...
if [ "$WITH_QODER_CLI" -eq 1 ] || [ "$WITH_QODER_CLI_CN" -eq 1 ] || [ "$WITH_KIMI_CLI" -eq 1 ]; then
    echo ""

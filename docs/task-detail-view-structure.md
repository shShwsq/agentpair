...
> 本文档描述**迭代摘要行("N 个工具调用: xxx")及其外框移除后**的目标结构。
...
2. **结果清单**:按 `task.params._grouping` 动态分组(如按严重度);卡片默认折叠,展开显示 Markdown 正文;文件类 meta 标签可点击打开左侧工作区文件。
3. **检查点评估聚合**:条目显示"第 N 轮 · 迭代 M"+ 继续/已打断徽标 + 理由(打断时附追问内容);点击定位对话流(见 §3.4)。
4. **任务概览**:状态徽标、导出按钮(Markdown/PDF,completed 或有结果时)、场景/创建时间/完成时间、用户意图(Markdown)、当前阶段、错误信息。
5. **动态验证**(配置了测试环境 URL 时):开关、授权模式切换、登录凭证(脱敏);不出现 verifier_agent 字样。
